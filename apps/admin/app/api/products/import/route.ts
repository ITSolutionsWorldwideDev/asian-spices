  ...
  try {
    ...
    for (const [index, row] of rows.entries()) {
      try {
        ...
        /* ---------- LOOKUPS ---------- */
        const categoryId = await resolveId(client, "store_categories", "name", row.Category);
        const subcategoryId = await resolveId(client, "store_subcategories", "name", row.Subcategory);
        const brandId = await resolveId(client, "store_brands", "name", row.Brand);

        const originCountryId = await resolveCountryId(client, row["Country of Origin"]);
...
async function resolveId(client: any, table: string, column: string, value?: string) {
