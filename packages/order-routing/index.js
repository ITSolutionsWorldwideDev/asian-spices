...
export const getOrderContext = async (client, orderId) => {
...
/* ================= STORE CANDIDATES ================= */
export const getCandidateStores = async (client, orderId, country) => {
    const { rows } = await client.query(`
    SELECT 
      spc.store_id,
...
/* ================= SORTING ================= */
export const sortStores = (stores, lat, lng) => {
    return stores
        .map((s) => ({
        ...s,
...
/* ================= ASSIGN DEFAULT ================= */
export const assignDefaultStore = async (client, orderId) => {
    console.log("assignDefaultStore DEFAULT_STORE_ID ===== ", DEFAULT_STORE_ID);
    if (!DEFAULT_STORE_ID) {
        throw new AppError("Default store not configured", "DEFAULT_STORE_MISSING", 500);
    }
    await client.query(`
    UPDATE store_orders
...
/* ================= ALLOCATIONS ================= */
const createAllocations = async (client, orderId, storeId) => {
    await client.query(`
...
/* ================= ORDER STATUS ================= */
export const resolveOrderStatus = async (client, orderId) => {
    const { rows } = await client.query(`
    SELECT 
...
/* ================= EVENTS ================= */
export const logOrderEvent = async (client, { orderId, eventType, storeId = null, message = "", metadata = {}, }) => {
    await client.query(`
...
/* ================= STORE STATUS ================= */
export const isStoreOpenNow = async (client, storeId) => {
    const now = new Date();
...
export const isTimeoutExceeded = async (client, attempt) => {
  ...
  `, [store_id]);
    ...
    while (cursor < now) {
        ...
        const hours = rows.find((r) => r.day_of_week === day);
...
/* ================= MAIN ROUTING ================= */
export const assignNextStore = async (client, orderId) => {
    ...
    // FULL STORE MATCH
    const fullStores = await getStoresThatCanFulfillAllItems(client, orderId, order.shipping_country);
    ...
    if (openStores.length > 0) {
        const sorted = sortStores(openStores, Number(order.shipping_latitude), Number(order.shipping_longitude));
        const { rows: attempts } = await client.query(`
      SELECT store_id
      FROM order_routing_attempts
      WHERE order_id = $1
    `, [orderId]);
        const tried = attempts.map((a) => a.store_id);
...
/* ================= MULTI STORE ================= */
export const assignMultiStore = async (client, orderId) => {
    const order = await getOrderContext(client, orderId);
...
/* ================= FULL FULFILLMENT ================= */
export const getStoresThatCanFulfillAllItems = async (client, orderId, country) => {
    const { rows } = await client.query(`
    SELECT 
      spc.store_id,
...
export const getStoresWithPartialItems = async (client, orderId, country) => {
    const { rows } = await client.query(`
    SELECT 
      spc.store_id,
      oi.product_id,
